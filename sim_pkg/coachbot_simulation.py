...
parser.add_argument('-fn', '--filename', type=str, help="Name of the file without the .py extension", required=True)
parser.add_argument('-c', '--configfile', type=str, required=True) # config
parser.add_argument('-i', '--initfile', type=str) # config
...
if args.initfile is None:
    ...
    init_poses = __import__(args.initfile)
    initted = True
...
def init_pos(swarmsize):
    ...
    if initted:
        x, y, theta, a_ids = init_poses.init(swarmsize,x,y,theta,a_ids) 
    else:
...
def run():
    ...
    with open(args.configfile + '.json', 'r') as myfile:
        data=myfile.read()
